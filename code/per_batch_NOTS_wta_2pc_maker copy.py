# updated version for analysing PER BATCH
# BEFORE RUNNING:
# 1. check input and output locations in slurm script
...
def main():
    ...
    parser.add_argument("--input", required=True, type=str, help="Path to the input batch directory") # should be a path to the batch directory
    parser.add_argument("--outdir", required=True, type=str, help="Directory where output should be saved")
    args = parser.parse_args()

    file_list = sorted(glob.glob(os.path.join(args.input, "*.root"))) # Get a list of all root files in that batch directory
    print(f"Found {len(file_list)} files to process in {args.input}")
    ...
    # -------------------------------------------------------------------------
    search_pattern = os.path.join(args.input, "batch*/*.root")
    ...
    # 2. Append the TTree name to each discovered file path for uproot
    tree_paths = [f"{filepath}:trackTree" for filepath in all_files]
    print("Beginning vectorized streaming iteration over all batches...")

    chunk_counter = 0
    # Pass the list of paths directly to uproot.iterate
    for data in uproot.iterate(tree_paths, expressions=["genJetPt", "genJetEta", "genDau_pt", "genDau_eta", "genDau_phi", "genDau_chg"], step_size="150 MB"):
        ...
        # MAKING THE SIGNAL, EPD, AND BACKGROUND (for this chunk) -----
        for bin_key in bin_pass_dict.keys():
            ...
            # BACKGROUND
            # --- WTA background ---
            if wta_n_sig_pairs > 0 and len(wta_epd_eta) > 0: # i.e. nonzero pairs
                n_wta_bg_pairs = 10 * wta_n_sig_pairs # from PRL paper
                idx1 = np.random.randint(0, len(wta_epd_eta), size=n_wta_bg_pairs) # random particle indices
                idx2 = np.random.randint(0, len(wta_epd_eta), size=n_wta_bg_pairs)
                wta_bg_deta = abs(wta_epd_eta[idx1] - wta_epd_eta[idx2]).astype(np.float64)
                wta_bg_dphi = np.arccos(np.cos(wta_epd_phi[idx1] - wta_epd_phi[idx2])).astype(np.float64)
                wta_bg_weights = np.ones_like(wta_bg_deta, dtype=np.float64)

                h_bg = wta_histograms[bin_key]["background"]
                h_bg.FillN(n_wta_bg_pairs,  wta_bg_deta,  wta_bg_dphi, wta_bg_weights)
                h_bg.FillN(n_wta_bg_pairs, -wta_bg_deta,  wta_bg_dphi, wta_bg_weights)
                h_bg.FillN(n_wta_bg_pairs,  wta_bg_deta, -wta_bg_dphi, wta_bg_weights)
                h_bg.FillN(n_wta_bg_pairs, -wta_bg_deta, -wta_bg_dphi, wta_bg_weights)
                h_bg.FillN(n_wta_bg_pairs,  wta_bg_deta, -wta_bg_dphi + 2 * PI, wta_bg_weights)
                h_bg.FillN(n_wta_bg_pairs, -wta_bg_deta, -wta_bg_dphi + 2 * PI, wta_bg_weights)
                
                # garbage collecting
                del wta_bg_deta, wta_bg_dphi, wta_bg_weights, idx1, idx2
                gc.collect()

            # STD background
            if std_n_sig_pairs > 0 and len(std_epd_eta) > 0:
                n_std_bg_pairs = 10 * std_n_sig_pairs
                idx1 = np.random.randint(0, len(std_epd_eta), size=n_std_bg_pairs)
                idx2 = np.random.randint(0, len(std_epd_eta), size=n_std_bg_pairs)
                std_bg_deta = abs(std_epd_eta[idx1] - std_epd_eta[idx2]).astype(np.float64)
                std_bg_dphi = np.arccos(np.cos(std_epd_phi[idx1] - std_epd_phi[idx2])).astype(np.float64)
                std_bg_weights = np.ones_like(std_bg_deta, dtype=np.float64)

                h_bg = std_histograms[bin_key]["background"]
                h_bg.FillN(n_std_bg_pairs,  std_bg_deta,  std_bg_dphi, std_bg_weights)
                h_bg.FillN(n_std_bg_pairs, -std_bg_deta,  std_bg_dphi, std_bg_weights)
                h_bg.FillN(n_std_bg_pairs,  std_bg_deta, -std_bg_dphi, std_bg_weights)
                h_bg.FillN(n_std_bg_pairs, -std_bg_deta, -std_bg_dphi, std_bg_weights)
                h_bg.FillN(n_std_bg_pairs,  std_bg_deta, -std_bg_dphi + 2 * PI, std_bg_weights)
                h_bg.FillN(n_std_bg_pairs, -std_bg_deta, -std_bg_dphi + 2 * PI, std_bg_weights)
                
                del std_bg_deta, std_bg_dphi, std_bg_weights, idx1, idx2
                gc.collect()
    ...
    for mult_bin in analysis_bins:
        ...
        # Calculate true combined average Nch for this bin across all streamed files
        avg_Nch_dict[bin_key] = {
            "wta": (total_Nch_sum_dict[bin_key]["wta"] / n_jets_wta) if n_jets_wta > 0 else 0.0,
            "std": (total_Nch_sum_dict[bin_key]["std"] / n_jets_std) if n_jets_std > 0 else 0.0
        }
        ...
        h_yield_wta.Reset() # start empty, fill if valid
        ...
        if n_jets_wta > 0 and h_sig_wta.GetEntries() > 0 and h_bg_wta.GetEntries() > 0:
            ...
            h_yield_wta.Add(h_sig_wta) # restore data from signal
        ...
        wta_yields[bin_key] = h_yield_wta # outside of the if statement - a hist should always save, even if empty
    ...
    # HARDCODE A DESCRIPTIVE FILE NAME SINCE THIS REPRESENTS ALL BATCHES
    full_output_path = os.path.join(args.outdir, "Analysis_Output_AllBatches.root")
    ...
    for mult_bin in analysis_bins:
        ...
        avg_Nch_wta = float(avg_Nch_dict[bin_key]["wta"])
        ...
        avg_Nch_std = float(avg_Nch_dict[bin_key]["std"])
        ...
        # Write TParameters
        param_avg_Nch_wta = ROOT.TParameter('double')(f"avg_Nch_WTA_{bin_name}", avg_Nch_wta)
        param_avg_Nch_std = ROOT.TParameter('double')(f"avg_Nch_STD_{bin_name}", avg_Nch_std)
        param_avg_Nch_wta.Write()
        param_avg_Nch_std.Write()
        print(f"Written master combined histograms for multiplicity bin: {bin_name}")

    out_file.Close()
    print(f"Results for ALL batches successfully written and saved to: {full_output_path}")
