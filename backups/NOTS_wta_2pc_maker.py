# updated version for analysing all files in a batch
# BEFORE RUNNING:
# 1. check input and output locations
...
def main():
    ...
    print(f"Beginning vectorized streaming iteration over path: {file_pattern}")

    chunk_counter = 0
    for data in uproot.iterate(file_pattern, expressions=["genJetPt", "genJetEta", "genDau_pt", "genDau_eta", "genDau_phi", "genDau_chg"], step_size="150 MB"):
        chunk_counter += 1
    ...
    batch_name = os.path.basename(args.input.strip("/"))
    full_output_path = os.path.join(args.outdir, f"Analysis_Output_{batch_name}.root")
    ...
    for mult_bin in analysis_bins:
        ...
        print(f"Written batch histograms for multiplicity bin: {bin_name}")

    out_file.Close()
    print(f"Results for batch {batch_name} written and saved")
